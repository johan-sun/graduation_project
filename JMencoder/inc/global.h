...
typedef struct
{
  ...
  //all_mv[b4x4_in_b8x8_y][b4x4_in_b8x8_x][list][ref][blocktype][x|y]
...
extern InputParameters *input;//保存输入配置文件的参数
extern ImageParameters *img;//迭代过程中保存各种临时信息，比如处理过程中的mv，对应的poc等等
...
#include<assert.h>
    ...
    {\
        char tmp_buf[100];\
        FILE* tmp_file;\
        sprintf(tmp_buf, fmt, ##args);\
        assert(tmp_file = fopen(tmp_buf, "w"));

...
int Mode_Decision_for_4x4IntraBlocks (int  b8,  int  b4,  double  lambda,  int*  min_cost)
{
  ...
  imgpel  rec4x4[4][4];
  double  rdcost;
  int     block_x     = 8*(b8 & 0x01)+4*(b4 & 0x01);//获得4x4块的左上角坐标x
  int     block_y     = 8*(b8 >> 1)+4*(b4 >> 1);//获得4x4块的左上角坐标y
  int     block_x4    = block_x >> 2;//以4x4为最小单位的坐标x
  int     block_y4    = block_y >> 2;//以4x4为最小单位的坐标y
  int     pic_pix_x   = img->pix_x+block_x;
  int     pic_pix_y   = img->pix_y+block_y;
...
{//cbp共6bit，高2bit表示cbpc（2：cb，cr中至少一个4x4块AC系数补全为0[有交流系数],1:cb,cr中至少一个2x2DC系数不全为0[有色度DC系数],0:所有系数全0
...
void store_macroblock_parameters (int mode)
{
  ...
  if (img->yuv_format != YUV400)
  {//mb_cr_size_y mb_cr_size_x 不同图片格式下的速度块的大小
    for (j = 0; j<img->mb_cr_size_y; j++)
    {
  ...
  //--- coeff, cbp, kac ---
  if (mode || bframe)
  {//swap cofAC with img->cofAC, cofDC with img->cofDC
...
void assign_enc_picture_params(int mode, int best_pdir, int block, int list_offset, int best_fw_ref, int best_bw_ref, int bframe)
{
  ...
  if (mode==1)
  {
    ...
    else 
    {
      for (j=0; j<BLOCK_MULTIPLE; j++)
      {
        block_y = img->block_y+(block&2)+j;//4x4block
        block_x = img->block_x+(block&1)*2;
        //enc_picture->ref_idx[LIST_0][block_y][block_x][0,1,2,3]=best_fw_ref
        memset(&enc_picture->ref_idx[LIST_0][block_y][block_x], best_fw_ref , BLOCK_MULTIPLE * sizeof(char));
        for (i=0; i<BLOCK_MULTIPLE; i++)//i<4
        {                                

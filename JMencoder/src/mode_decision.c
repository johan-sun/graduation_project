                                 ...
                                 int transform8x8)  
{
  ...
  short bi_pred_me;
  int best_cnt_nonz = 0;
  int maxindex =  (transform8x8) ? 2 : 5;//transform8x8=0时会计算8x4，4x8，4x4，否则掠过这3个
  int pix_x, pix_y;
  int block_x, block_y;
  ...
  //--- set coordinates ---
  //i0,j0 for block = 0,1,2,3 is
  //			(0,0),(16,0),(0,16),(16,16)
  //i,j is	(0,0),(8,0) ,(0,8 ),(8,8)
...
{  
  ...
  short       best_pdir;//最佳模式的方向
  RD_PARAMS   enc_mb;//存放原来8.6里面的参数,包括valid数组，拉格朗日参数等
  double      min_rdcost, max_rdcost=1e30;
  char        best_ref[2] = {0, -1};
  int         bmcost[5] = {INT_MAX};//motion_cost + lambda_mostion*rate(ret)
  int         cost=0;
  ...
  //allmvs以4x4块为单位存放运动矢量，初始化指向list0，ref=0的第一个4x4块
  ...
  init_enc_mb_params(currMB, &enc_mb, intra, bslice);//初始化enc_mb参数，设置了valid，拉格朗日参数等
  ...
  for (rerun=0; rerun<runs; rerun++)
  {
      ...
      for (min_cost=INT_MAX, mode=1; mode<4; mode++)
      {
        ...
        if (enc_mb.valid[mode] && !inter_skip)
        {
          ...
          for (cost=0, block=0; block<(mode==1?1:2); block++)
          {
            ...
            //img->all_mv被更新
            //motion_cost被更新
            //enc_picture->mv[list][b4x4-y][b4x4-x][x|y]被更新 //已经指向当前mode对应的mv
            //enc_picture->ref_idx被更新//已经指向当前mode对应idx
            ...
            //get cost and reference frame
            //bmcost best_motion_cost[list]对应list最佳cost，通过比较
            //通过遍历对应list里面的motion_cost与当前bmcost[list]对比
            //将小的值存放在bmcost[list]中 best motion cost(已经计算了lambda * rate（ref）
            //最佳引用存放在best_ref[list]
            list_prediction_cost(LIST_0, block, mode, enc_mb, bmcost, best_ref);
          ...
          //如果当前mode下16x16的cost<min_cost则
          //宏块级的最佳mode就是当前mode记录mode与min_cost
   ...
   if (input->rdopt)
   {
    ...
    if (!inter_skip)
    {
        ...
        {
          // precompute all new chroma intra prediction modes,已经将所有帧内预测模式下的值存放在img->mprr_c数组中
